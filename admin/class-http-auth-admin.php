...
class HTTP_Auth_Admin
{
     ...
     */
    private $css_suffix = '-' . HTTP_AUTH_PLUGIN_VERSION . '.min.css';
    ...
    {
        add_menu_page('HTTP Auth', 'HTTP Auth', 'administrator',
            'http-auth-settings', array( $this, 'admin_settings_page' )
        );
        $settings_page = add_submenu_page( 'http-auth-settings',
            'HTTP Auth Settings', 'Settings', 'administrator',
            'http-auth-settings', array( $this, 'admin_settings_page' )
        );
        $about_page = add_submenu_page( 'http-auth-settings', 'About HTTP Auth',
            'About', 'administrator', 'http-auth-about-plugins',
            array( $this, 'about_plugin' )
        );

        add_action( 'admin_print_styles-' . $settings_page . '',
    ...
    public function add_about_style()
    {
        ...
        wp_enqueue_style( 'http-auth-about-style',
    ...
    public function add_settings_page_style()
    {
        ...
        $plugin_url = plugins_url( '/admin', HTTP_AUTH_FILE );
        wp_enqueue_style( 'http-auth-settings-style',
            $plugin_url . '/css/' . $filename
        );
    }
    ...
    public function admin_footer_text()
    {
        $footer_text = sprintf(
            __(
                'HTTP Auth version %s by <a href="%s" title="YAS Global Website" target="_blank">YAS Global</a> - <a href="%s" title="Support forums" target="_blank">Support forums</a> - Follow on Twitter: <a href="%s" title="Follow YAS Global on Twitter" target="_blank">YAS Global</a>',
                'http-auth'
            ),
    ...
    public function settings_link( $links )
    {
        ...
        $contact = sprintf(
            __( '<a href="%s" title="Contact" target="_blank">Contact</a>',
                'http-auth'
            ),
